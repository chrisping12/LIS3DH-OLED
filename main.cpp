...
/* Display the results (acceleration is measured in m/s^2) */
    ...
    display.println(event.acceleration.x,2);
    display.setCursor(63,35);
    display.println(event.acceleration.y,2);
    display.setCursor(63,52);
    display.println(event.acceleration.z,2);
}
...
if(!digitalRead(BUTTON_A)){
  drawPingLogo(); display.display();
}
  
if(!digitalRead(BUTTON_B)){
  display.clearDisplay();
  display.drawBitmap(0,0,PingBling,LOGO_WIDTH,LOGO_HEIGHT,1);
  display.display();
  delay(8000);
  display.clearDisplay();
  display.display();
